...
def reorient_to_team_convention(img: nib.Nifti1Image) -> nib.Nifti1Image:
    ...
    Reorient volume to the team's view convention used in downstream inspection:
      ...
      - axis 1: axial index (with direction flip to match expected angle)
      - axis 2: sagittal index

    This avoids repeated ad-hoc axis/rotation fixes in notebook visualization.
    """
    data = img.get_fdata(dtype=np.float32)
    src_ornt = io_orientation(img.affine)

    # Target orientation expressed in source-axis indices/signs.
    # [ [1,+1], [2,-1], [0,+1] ] ~= (coronal, axial flipped, sagittal)
    dst_ornt = np.array([[1.0, 1.0], [2.0, -1.0], [0.0, 1.0]], dtype=np.float64)
