...
Input volumes are assumed to be in RAS canonical orientation:
  - axis 0 = sagittal
  - axis 1 = coronal
  - axis 2 = axial

Export convention for downstream 2D data:
  - export axis 0 = coronal  (source RAS axis 1) -> rot180
  - export axis 1 = axial    (source RAS axis 2) -> unchanged
  - export axis 2 = sagittal (source RAS axis 0) -> rot90 counter-clockwise
"""
...
EXPORT_VIEWS: tuple[dict[str, int | str], ...] = (
    {"export_axis": 0, "name": "coronal", "source_axis": 1},
    {"export_axis": 1, "name": "axial", "source_axis": 2},
    {"export_axis": 2, "name": "sagittal", "source_axis": 0},
)
...
def _orient_slice_for_export(slice_2d: np.ndarray, export_axis: int) -> np.ndarray:
    if export_axis == 0:
...
def main() -> None:
    ...
    for fold_name, split_name, class_name, nii_path in tqdm(samples, desc="Volumes"):
        ...
        for view in EXPORT_VIEWS:
            export_axis = int(view["export_axis"])
            ...
            for idx in range(start, end, args.slice_step):
                ...
                slice_2d = _orient_slice_for_export(slice_2d, export_axis)
