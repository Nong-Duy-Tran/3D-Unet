...
if [[ $# -lt 1 ]]; then
  ...
  echo "  label_mode -> cdr4 | normal_vs_abnormal | ad_vs_nonad | cn_vs_ad_drop_05"
  exit 1
fi

VERSION="$1"
LABEL_MODE="${2:-normal_vs_abnormal}"
...
case "$VERSION" in
  1)
    ...
    OUTPUT_DIR_3D="data/processed_oasis_3d_cv5_v1"
    OUTPUT_DIR_2D="data/processed_oasis_2d_cv5_v1"
    ;;
  2)
